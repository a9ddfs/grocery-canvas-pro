...
import { TrendingUp, DollarSign, ShoppingBag, AlertTriangle, Package, ArrowRight } from "lucide-react";
  ...
  const categoryData = useMemo(() => {
    ...
    sales.forEach((s) => {
      s.items.forEach((item) => {
        const catName = item.category === "fruits" ? "فواكه" :
          item.category === "vegetables" ? "خضروات" :
          item.category === "dairy" ? "ألبان" :
          item.category === "bakery" ? "مخبوزات" :
          item.category === "drinks" ? "مشروبات" : "معلبات";
    ...
    <div className="h-screen flex flex-col bg-background overflow-hidden" dir="rtl">
      ...
      <div className="flex-1 overflow-y-auto p-6 pos-scrollbar space-y-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <StatCard icon={<DollarSign className="w-5 h-5" />} label="مبيعات اليوم" value={`${stats.todayRevenue.toFixed(0)} ر.س`} color="primary" />
          <StatCard icon={<ShoppingBag className="w-5 h-5" />} label="طلبات اليوم" value={`${stats.todaySales}`} color="success" />
          <StatCard icon={<Package className="w-5 h-5" />} label="منتجات مباعة اليوم" value={`${stats.todayItems}`} color="info" />
          <StatCard icon={<AlertTriangle className="w-5 h-5" />} label="مخزون منخفض" value={`${stats.lowStock}`} color="destructive" onClick={() => onNavigate("inventory")} />
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="bg-card rounded-xl border border-border p-5">
            <h3 className="font-bold text-foreground mb-4">المبيعات الأسبوعية</h3>
            <ResponsiveContainer width="100%" height={250}>
              <AreaChart data={dailyData}>
                ...
                <Area type="monotone" dataKey="revenue" stroke="hsl(36, 95%, 55%)" fill="url(#colorRevenue)" strokeWidth={2} name="الإيرادات" />
              </AreaChart>
            </ResponsiveContainer>
          </div>
          ...
          {/* Orders per day */}
          <div className="bg-card rounded-xl border border-border p-5">
            <h3 className="font-bold text-foreground mb-4">الطلبات اليومية</h3>
            <ResponsiveContainer width="100%" height={250}>
              <BarChart data={dailyData}>
                ...
                <YAxis stroke="hsl(220, 10%, 55%)" fontSize={12} />
                <Tooltip contentStyle={{ background: "hsl(220, 18%, 14%)", border: "1px solid hsl(220, 15%, 22%)", borderRadius: "8px", color: "hsl(40, 20%, 95%)" }} />
                <Bar dataKey="orders" fill="hsl(142, 71%, 45%)" radius={[6, 6, 0, 0]} name="الطلبات" />
              </BarChart>
            </ResponsiveContainer>
          </div>
          ...
          {/* Category breakdown */}
          <div className="bg-card rounded-xl border border-border p-5">
            <h3 className="font-bold text-foreground mb-4">المبيعات حسب التصنيف</h3>
            <ResponsiveContainer width="100%" height={250}>
              <PieChart>
          ...
          {/* Payment methods */}
          <div className="bg-card rounded-xl border border-border p-5">
            <h3 className="font-bold text-foreground mb-4">طرق الدفع</h3>
            <ResponsiveContainer width="100%" height={250}>
              <PieChart>
        ...
        {/* Recent sales */}
        <div className="bg-card rounded-xl border border-border p-5">
          <h3 className="font-bold text-foreground mb-4">آخر المبيعات</h3>
              ...
              <div key={sale.id} className="flex items-center justify-between py-2.5 px-3 rounded-lg bg-secondary/50 text-sm">
                ...
                <div className="flex items-center gap-4">
                  <span className="text-muted-foreground text-xs">
                    {new Date(sale.date).toLocaleTimeString("ar-SA", { hour: "2-digit", minute: "2-digit" })}
                  </span>
