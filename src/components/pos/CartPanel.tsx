...
interface CartPanelProps {
  ...
  onClear: () => void;
  onCompleteSale: (method: "cash" | "card") => void;
}

const CartPanel = ({ items, total, itemCount, onUpdateQuantity, onRemove, onClear, onCompleteSale }: CartPanelProps) => {
          ...
          {!showCheckout ? (
            <>
              <div className="space-y-1.5 text-sm">
                <div className="flex justify-between text-muted-foreground">
                  <span>المجموع الفرعي</span><span>{total.toFixed(2)} ر.س</span>
                </div>
                <div className="flex justify-between text-muted-foreground">
                  <span>ضريبة القيمة المضافة (15%)</span><span>{tax.toFixed(2)} ر.س</span>
                </div>
                <div className="flex justify-between text-foreground font-bold text-lg pt-2 border-t border-border">
                  <span>الإجمالي</span><span className="text-primary">{grandTotal.toFixed(2)} ر.س</span>
                </div>
              </div>
            ...
            <div className="space-y-3 animate-slide-in">
              ...
              <div className="grid grid-cols-2 gap-3">
                <button onClick={() => handlePay("cash")} className="flex flex-col items-center gap-2 py-4 rounded-lg bg-pos-success/15 border border-pos-success/30 hover:bg-pos-success/25 transition-colors">
                  <Banknote className="w-8 h-8 text-pos-success" />
                  <span className="text-sm font-semibold text-foreground">نقدي</span>
                </button>
                <button onClick={() => handlePay("card")} className="flex flex-col items-center gap-2 py-4 rounded-lg bg-pos-info/15 border border-pos-info/30 hover:bg-pos-info/25 transition-colors">
                  <CreditCard className="w-8 h-8 text-pos-info" />
                  <span className="text-sm font-semibold text-foreground">بطاقة</span>
                </button>
              </div>
              <button onClick={() => setShowCheckout(false)} className="w-full text-muted-foreground text-sm py-2 hover:text-foreground transition-colors">
                رجوع
              </button>
            </div>
          )}
