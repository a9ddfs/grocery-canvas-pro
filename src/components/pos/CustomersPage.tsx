...
import { Users, Plus, Phone, MapPin, Edit2, Trash2, X, DollarSign, Calendar, CheckCircle2, AlertTriangle } from "lucide-react";
    ...
    <div className="h-screen flex flex-col bg-background overflow-hidden" dir="rtl">
      ...
      <div className="flex-1 overflow-y-auto p-6 pos-scrollbar">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-1 space-y-3">
            <h3 className="font-bold text-foreground text-sm">العملاء ({customers.length})</h3>
            {customers.map((cust) => {
          ...
          <div className="lg:col-span-2 space-y-4">
            {selectedCustomer ? (
              <>
                ...
                <div className="bg-card rounded-xl border border-border p-5">
                  ...
                  {getCustomerDebts(selectedCustomer).length === 0 ? (
                    <p className="text-muted-foreground text-sm text-center py-4">لا توجد ديون</p>
                  ) : (
                          ...
                          <div className="flex items-center justify-between mb-2">
                            <div className="flex items-center gap-2">
                              <span className={`px-2 py-0.5 rounded text-xs font-semibold ${
                                ...
                                {debt.status === "paid" ? "مسدد" : debt.status === "partial" ? "جزئي" : "معلق"}
                              </span>
                              <span className="text-xs text-muted-foreground flex items-center gap-1">
                                <Calendar className="w-3 h-3" />
                                الاستحقاق: {new Date(debt.dueDate).toLocaleDateString("ar-SA")}
                              </span>
                            </div>
                            <span className="font-bold text-foreground">{debt.amount.toFixed(2)} ر.س</span>
                          </div>
                          <div className="flex items-center justify-between text-xs">
                            <span className="text-muted-foreground">المدفوع: {debt.paidAmount.toFixed(2)} | المتبقي: {(debt.amount - debt.paidAmount).toFixed(2)}</span>
                            {debt.status !== "paid" && (
                            ...
                            <div className="mt-2 pt-2 border-t border-border space-y-1">
                              {debt.payments.map((p) => (
                                <div key={p.id} className="flex justify-between text-xs text-muted-foreground">
                                  <span>دفعة: {p.amount.toFixed(2)} ر.س</span>
                ...
                {/* Purchase History */}
                <div className="bg-card rounded-xl border border-border p-5">
                  <h3 className="font-bold text-foreground mb-4">سجل المشتريات</h3>
                  {getCustomerPurchases(selectedCustomer).length === 0 ? (
                    <p className="text-muted-foreground text-sm text-center py-4">لا توجد مشتريات</p>
                  ) : (
                    <div className="space-y-2">
                      {getCustomerPurchases(selectedCustomer).slice(0, 10).map((sale) => (
                        <div key={sale.id} className="flex items-center justify-between py-2 px-3 rounded-lg bg-secondary/50 text-sm">
                          <span className="text-muted-foreground">{sale.items.length} منتج</span>
                          <div className="flex items-center gap-3">
                            <span className="text-xs text-muted-foreground">{new Date(sale.date).toLocaleDateString("ar-SA")}</span>
                            <span className="font-bold text-primary">{sale.grandTotal.toFixed(2)} ر.س</span>
                          </div>
                        </div>
                      ))}
                    </div>
              ...
              <div className="bg-card rounded-xl border border-border p-12 text-center">
                ...
                <p className="text-muted-foreground">اختر عميلاً لعرض التفاصيل</p>
              </div>
            )}
          </div>
        </div>
      ...
      {showAdd && (
        <div className="fixed inset-0 bg-background/80 backdrop-blur-sm z-50 flex items-center justify-center p-4" onClick={() => { setShowAdd(false); setEditingId(null); }}>
          <div className="bg-card rounded-2xl border border-border w-full max-w-md p-5 animate-pop-in" dir="rtl" onClick={(e) => e.stopPropagation()}>
            <h2 className="font-bold text-foreground text-lg mb-4">{editingId ? "تعديل العميل" : "إضافة عميل جديد"}</h2>
            <div className="space-y-3">
              <input placeholder="اسم العميل" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} className="w-full bg-secondary text-foreground px-3 py-2.5 rounded-lg border border-border text-sm" />
      ...
      {payDebtId && (
        <div className="fixed inset-0 bg-background/80 backdrop-blur-sm z-50 flex items-center justify-center p-4" onClick={() => setPayDebtId(null)}>
          <div className="bg-card rounded-2xl border border-border w-full max-w-sm p-5 animate-pop-in" dir="rtl" onClick={(e) => e.stopPropagation()}>
            <h2 className="font-bold text-foreground text-lg mb-4">تسديد دين</h2>
            <div className="space-y-3">
              <div>
